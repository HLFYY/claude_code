...
function customEncode(e) {
  ...
  for (let l = 0; l < e.length; l++) {
    ...
    s = l % 6;
    switch (s) {
      case 0: c.push(F_CHARSET.charAt(a >> 2)); break;
      case 1: c.push(F_CHARSET.charAt(((2 & o) << 3) | (a >> 4))); break;
      case 2:
        c.push(F_CHARSET.charAt(((15 & o) << 2) | (a >> 6)));
        c.push(F_CHARSET.charAt(a & 63));
        break;
      case 3: c.push(F_CHARSET.charAt(a >> 3)); break;
      case 4: c.push(F_CHARSET.charAt(((o & 4) << 6) | (a >> 6))); break;
      case 5:
        c.push(F_CHARSET.charAt((o & 15) << 4));
        c.push(F_CHARSET.charAt(a & 63));
        break;
    }
    o = a;
  }
  if (s === 0) { c.push(F_CHARSET.charAt((o & 3) << 4)); c.push('FM'); }
  else if (s === 1) { c.push(F_CHARSET.charAt((15 & o) << 2)); c.push('K'); }
  return c.join('');
}
...
function randomStr(len) {
  let s = '';
  for (let i = 0; i < len; i++)
    s += D_CHARSET.charAt(Math.floor(Math.random() * D_CHARSET.length));
  return s;
}

// ── 5. 生成 m 参数 ───────────────────────────────────────────────────────────
function generateM(t) {
  const encoded = customEncode(t);
  const hash    = customMd5(encoded);
  return randomStr(15) + hash + randomStr(10);
}
...
function fetchPage(page) {
  return new Promise((resolve, reject) => {
    ...
    const m   = generateM(t);
    const url = `https://match.yuanrenxue.cn/api/question/16?page=${page}&m=${encodeURIComponent(m)}&t=${t}`;
    console.log(`\n[页面 ${page}] t=${t}`);
    console.log(`  m=${m}`);
    ...
    https.get(url, {
      headers: {
        ...
        'Cookie':           'sessionid=1a5berh9e1hkhrflw8fv5d9sdo7igb8n',
      },
    }, (res) => {
      let data = '';
...
async function main() {
  ...
  console.log('=== 编码自测 ===');
  const encTests = {
    '1774344582000': 'y7iE4OHx7qS4UUOyUFM',
    '1774344029000': 'y7iE4OHx7UA3UUOyUFM',
  };
  for (const [ts, exp] of Object.entries(encTests)) {
    const got = customEncode(ts);
    console.log(`  ${got===exp?'✓':'✗'} encode("${ts}") = "${got}"`);
  }

  // 自测 MD5
  console.log('\n=== MD5 自测 ===');
  const md5Tests = {
    'y7iE4OHx7qS4UUOyUFM': 'afd04c4df1bb04951de7c93403e8f6a1',
    'y7iE4OHx7UA3UUOyUFM': 'bfa05891cb6c8f22838245c2523df2fe',
  };
  for (const [input, exp] of Object.entries(md5Tests)) {
    const got = customMd5(input);
    console.log(`  ${got===exp?'✓':'✗'} md5("${input}") = "${got}"`);
  }
  ...
  if (result && result.data) {
    console.log(`\n✓ 第1页数字: [${result.data.join(', ')}]`);
    console.log(`  数字之和: ${result.data.reduce((a, b) => a + b, 0)}`);
  }
}
