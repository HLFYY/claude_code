...
每次大概率都不一样，手动扒一次不划算），而是直接把这段 <script> 丢进一个极简的 Node
vm 沙箱真实跑一遍，拿它自己产生的 document.cookie 赋值结果（做法跟 tdc_bridge.js 一致），
可读做法见 eo_challenge_bridge.js。

已用真实拦截页验证过：还原出的 __tst_status=538622729 跟这个站点其他地方（demo.py 里
之前手动登录时抓到的 cookie）出现过的同一个值完全一致，eo_challenge_bridge.js 的输出
是可信的。
"""
...
def solve(html: str) -> dict[str, str]:
    ...
    for write in cookie_writes:
        # "__tst_status=538622729#;" / "EO_Bot_Ssid=969736192;" 这种，去掉结尾的 ';'
        # 和 document.cookie 惯用的 path/expires 部分（这个站点的挑战脚本没带那些，
        # 但留着这个清理逻辑更稳)。
...
def prime(session: requests.Session, url: str, **kwargs) -> dict[str, str]:
    """主动预热一次：请求 url，如果撞上挑战就解掉、把 __tst_status/EO_Bot_Ssid 这两个
    cookie 存进 session.cookies 并返回；没撞上就返回空 dict。

    实测这两个 cookie 一旦拿到手，后面同一个 session 发的请求就不会再撞挑战了
    （不是每次请求都要重新解一遍）——所以合理的用法是登录成功后主动调一次这个函数
    （而不是靠 get_with_challenge_retry 被动等第一次真实请求撞上再解），后面所有
    请求直接用这同一个 session 就带上了，不需要每次都走"请求->判断->解->重试"这套。
    """
...
if __name__ == "__main__":
    sample = Path("/tmp/eo_test.js")
    if sample.exists():
        print(solve(f"<script>{sample.read_text()}</script>"))
